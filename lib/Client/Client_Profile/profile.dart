...
import 'image_info_setting.dart';
import 'notification.dart';
import 'privacy_setting_client.dart';
          ...
          onTap: () {
            ...
            if (label == 'Privacy') {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => PrivacySettingClientScreen()),
              );
            }
            if (label == 'Feedback') {
              Navigator.push(
...
Future<void> _updateProfilePicture(BuildContext context) async {
  // Implement your update logic here, for example, show a dialog to select a new image
  // For simplicity, let's assume imagePath contains the path to the selected image
  String imagePath =
      'path_to_selected_image.jpg'; // Replace with the actual path
  await updateProfilePicture(imagePath);
  // Optional: Show a snackbar or toast to indicate that the profile picture has been updated
  ScaffoldMessenger.of(context)
      .showSnackBar(SnackBar(content: Text('Profile picture updated')));
}
